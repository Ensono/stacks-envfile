...
Write-Host ("Buildnumber: {0}" -f $BuildNumber)
...
foreach ($os in $targets) {
    ...
    foreach ($_arch in $archs) {
        ...
        $cli_filename = "{0}/stacks-envfile-{1}-{2}-{3}" -f $BasePath, $os, $_arch, $BuildNumber
        $inttest_filename = "{0}/stacks-envfile-{1}-inttest-{2}-{3}" -f $BasePath, $os, $_arch, $BuildNumber
        ...
        $cmd = 'go build -ldflags "-X github.com/amido/stacks-envfile/main.Version={0}" -o {1}' -f
                    $BuildNumber,
                    $cli_filename
